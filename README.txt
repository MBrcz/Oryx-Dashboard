...
In short:
The idea is to create to the dashboard that will utilize data collected by Oryx Blog that contains data of visually confirmed losses in the Second Russo-Ukrainian War (2022+) in specified points in time (right now every quarter till start of conflict and when wayback machine started visiting Oryx blog).
...
-------------------
Update 2 (4/18/2024):
Reworked version in Excel due to it's instability and hostilness towards end user. Good news is that now, without macros turned on, user can see the base version of the dashboard, but news is that there is NO fullscreen mode applied.